...
class TestGatherBounded:
    ...
    @pytest.mark.parametrize(
        ("n", "total"),
        [
            (1, 5),  # n=1 takes the sequential short-circuit, not the semaphore
            (4, 20),
        ],
    )
    async def test_n_caps_in_flight(self, n: int, total: int):
        in_flight = 0
        max_in_flight = 0
        ...
        coros: list[Awaitable[None]] = [make_coro() for _ in range(total)]
        await config._gather_bounded(n, coros)
